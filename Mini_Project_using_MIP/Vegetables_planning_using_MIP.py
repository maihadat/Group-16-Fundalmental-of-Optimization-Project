...
def vegetables_planning(filename, print_out_plan, print_out_objective, print_out_time):
    ...
    INF = solver.infinity()
    x = [[solver.IntVar(0.0, 1.0, 'x(' + str(i) + ',' + str(j) + ')') for j in range(1, J + 1)] for i in
         range(1, N + 1)]
    ...
    # con6: Rằng buộc để cân bằng công suất các ngày bằng cách cho z min
    for j in range(J):
        con6 = solver.Constraint(0, INF)
        con6.SetCoefficient(z, 1)
        con6.SetCoefficient(y[j], -1)
...
vegetables_planning(input("Nhập filename: "), True, True, True)
